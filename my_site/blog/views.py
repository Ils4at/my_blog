...
def post_detail(request, year, month, day, post):
    posts = get_object_or_404(Post, status=Post.Status.PUBLISHED,
                              ...
                              publish__day=day)
    # Список активных комментариев к этому посту набор запросов QuerySet
...
class PostListViews(ListView):
    # Альтернативное представление портов
...
def post_share(request, post_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            send_mail(subject, message, 'sharipov3000@gmail.com',
                      [cd['to']])
            sent = True
    else:
        form = EmailPostForm()
...
def post_comment(request, post_id):
    # Создание представления для комментариев
    post = get_object_or_404(Post,
                             id=post_id,
                             status=Post.Status.PUBLISHED)
    comment = None
    ...
    if form.is_valid():
        ...
        comment.post = post
        # Сохранять коментарий в базе данных
        comment.save()
